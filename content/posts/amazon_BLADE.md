...
아마존에서는 BLADE를 이용해 모든 노드에 대해서 소스 및 타겟 임베딩 생성한 다음엔 기존의 추천 시스템과 동일한 과정을 거치고 있다. 우리의 목표는 상품이 주어졌을 때, 그 상품과 구매될 가능성이 높은 상위 제품이 무엇인지를 찾는 것. 즉, 쿼리 상품 Q를 임베딩 공간에 매핑하고, 잠재적 후보를 타겟 임베딩 공간에 매핑한 뒤 추천 노드를 찾기 위해 최근접 이웃 탐색을 진행한다.
